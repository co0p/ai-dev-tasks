---
name: generate-implementation-prompt
argument-hint: none
---

# Prompt: Generate Self-Contained implementation.prompt.md

You are tasked with generating a fully self-contained `implementation.prompt.md` file for the 4DC project. Use the contents of all files in the `/templates/implement` folder (including frontmatter.md, persona.md, goal.md, process.md, interaction.md, output.md, style.md, glossary.md, acceptance.md, examples.md, schema-hints.md) as source material.

- Only merge content from the specified template files. Do not add extra summary or instructional lines unless they are present in the templates.
- Do not append commentary like "This file is now ready to use..." unless it is explicitly included in the template files.
- The frontmatter (YAML block) from `frontmatter.md` must be placed at the very top of the generated file, before any other content.
- Merge order (top to bottom): frontmatter.md, persona.md, goal.md, process.md, interaction.md, output.md, style.md, glossary.md, acceptance.md, examples.md, schema-hints.md.
- Do not reference or link to any external files or folders in the output.
- Constraints: Human-first interaction; any structured outputs (JSON) are internal-only for tooling/CI.
- The resulting prompt must include the persona, process, interaction style, and output format directly in the file.
- Ensure clarity, completeness, and maintainability.
- The output should be ready to use as a standalone prompt for generating an implementation plan and code for an increment.

# Output
A single, self-contained `implement.prompt.md` file containing all necessary instructions, context, and templates for implementation generation. The file should be created at the top level of the repository (e.g., `/implement.prompt.md`).
