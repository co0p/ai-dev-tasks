...
- The **Constitution** sets values, principles, and constraints (WHY), including preferences for deployment frequency, testing, and operability.
- Each **Increment** states a new hypothesis/goal (WHAT), always framed by the constitution and scoped for small, testable, releasable changes.
- **Design** is mandatory and produced via the Design prompt (`design.prompt.md`); it establishes approach and boundaries (HOW), including test strategy, deployment/rollback, and observability before implementation.
- **Implementation** plan is generated by the Implement prompt (`implement.prompt.md`) and then executed (DO), emphasizing tests, CI checks, and small PRs.
- **Improve** prompt (`improve.prompt.md`) extracts learnings, recommends refactors and operational improvements, and promotes decisions into ADRs (LEARN), directly impacting change failure rate and MTTR.
